...
const Home = () => {
  ...
  reverseOrder={false}
/>
      <nav className="bg-white border-gray-200 dark:bg-cyan-100">
    ...
    <span className="text-xs text-gray-800 dark:text-gray-400 absolute start-0 -bottom-6">Below - {price} Rs.</span>
          ...
          <div className="flex">
            ...
            <BiCartAlt className="ms-4 mt-2" style={{ fontSize: "2rem" }} />{" "}
            <p className="text-2xl ms-1 mt-2">{addToCart.length}</p>
      ...
      <div  style={{marginTop:"30px"}} className="flex flex-wrap">
        {data.length == 0 ? (
          <div className="mx-auto"><p className="text-3xl mt-8 font-semibold text-center">NO PRODUCTS</p></div>
        ) : (
          data.map((item) => {
            return (
              <div key={item.id} className="w-full max-w-sm  bg-white ms-8 mb-6 border border-gray-200 rounded-lg shadow dark:bg-cyan-100 dark:border-gray-700">
                <a href="#">
                  <img
                ...
                <div className="px-5 pb-5">
                  <a href="#">
                    <h5 className="text-xl font-semibold tracking-tight font-bold text-gray-900 dark:text-black">
                      {item.title}
                    </h5>
                  </a>
